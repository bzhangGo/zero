...
def beam_search(features, encoding_fn, decoding_fn, params):
    ...
    if params.search_mode == "cache":
        model_state = encoding_fn(features["source"], features["source_mask"], features["sent_source"], features["sent_source_mask"])
    else:
    ...
    # [batch, "x"]
    prefix = features["target"]
    init_seq = util.expand_tile_dims(prefix, beam_size, axis=1)
    # init_seq = tf.fill([batch_size, beam_size, 1], params.tgt_vocab.pad())
    init_finish_seq = init_seq
    # [batch, beam]
    ...
    def cache_init(prev_seq, state):
        ...
        _, step_state = decoding_fn(
            flat_prev_seqs, flat_prev_state, 0, 0)
    ...
    for layer in range(params.num_decoder_layer):
        ...
        model_state['doc']['decoder']['state'][name]['k'] = model_state['doc']['decoder']['state'][name]['k'][:, :, :-1, :]
        model_state['doc']['decoder']['state'][name]['v'] = model_state['doc']['decoder']['state'][name]['v'][:, :, :-1, :]

    model_state['sent'] = util.dict_update(model_state['sent'], old_model_state['sent'])

    time_offset = util.shape_list(features['target'])[1] - 1
    ...
    def _step_fn(time, bsstate):
        ...
        step_logits, step_state = decoding_fn(
            decode_target, flat_prev_state, time, time_offset)
    ...
    time += util.shape_list(features['target'])[1] - 1 # + pad_length
    ...
    tlen = util.shape_list(features["target"])[1]
