...
def encoder(source, to_lang, p):
  ...
  with tf.variable_scope("encoder"):
    ...
    for layer in range(p.num_encoder_layer):
      ...
      with tf.variable_scope("layer_{}".format(layer), initializer=layer_init):
        with tf.variable_scope("self_attention"):
          ...
          x = robt.layer_norm(x, to_lang, p.to_lang_vocab.size(), use_tpu=p.use_tpu)
    ...
    x = robt.lang_mapper(x, to_lang, p.to_lang_vocab.size(), use_tpu=p.use_tpu)
  ...
  with tf.variable_scope("decoder"):
    ...
    for layer in range(p.num_decoder_layer):
      if p.deep_transformer_init:
        ...
        layer_init = None
      with tf.variable_scope("layer_{}".format(layer), initializer=layer_init):
        # merged attention: for faster training and generation
        if p.enable_fuse:
...
def train_fn(features, initializer=None):
  ...
  with tf.variable_scope(p.scope_name or "model",
    ...
    state = encoder(features['source'], features['to_lang'], p)
