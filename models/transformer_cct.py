...
def encoder(source, p, gater):
  ...
  # for CCT model, adopt pre-normalization structure
  # to ease token-layer pruning
    ...
    for layer in range(p.num_encoder_layer):
      ...
      with tf.variable_scope("layer_{}".format(layer), initializer=layer_init):
        with tf.variable_scope("self_attention"):
          ...
          gate["enc_san_query_l%d" % layer] = y['query_p_c']
          gate["enc_san_memory_l%d" % layer] = y['memory_p_c']

          y = y['output']
          x = func.residual_fn(x, y, dropout=p.residual_dropout)

        with tf.variable_scope("feed_forward"):
          y, ffn_p_c = cct.cct_ffn_layer(
            func.layer_norm(x),
            p.filter_size,
            hidden_size,
            p.cct_M,
          ...
          gate["enc_ffn_l%d" % layer] = ffn_p_c
          x = func.residual_fn(x, y, dropout=p.residual_dropout)

    x = func.layer_norm(x, scope="enc_output_norm")
...
def decoder(target, state, p, gater):
    ...
    for layer in range(p.num_decoder_layer):
      ...
      with tf.variable_scope("layer_{}".format(layer), initializer=layer_init):
        with tf.variable_scope("self_attention"):
          ...
          gate["dec_san_query_l%d" % layer] = y['query_p_c']
          gate["dec_san_memory_l%d" % layer] = y['memory_p_c']
        ...
        with tf.variable_scope("cross_attention"):
          ...
          gate["dec_can_query_l%d" % layer] = y['query_p_c']
          gate["dec_can_memory_l%d" % layer] = y['memory_p_c']
        ...
        with tf.variable_scope("feed_forward"):
          ...
          gate["dec_ffn_l%d" % layer] = ffn_p_c
          x = func.residual_fn(x, y, dropout=p.residual_dropout)

    x = func.layer_norm(x, scope="dec_output_norm")
  ...
  # conditional budget loss
  if is_training:
    enc_budget, enc_all_budget = cct.cct_budget(state['gate'], state['mask'], state['mask'])
    dec_budget, dec_all_budget = cct.cct_budget(gate, mask, state['mask'])

    used_budget = enc_budget + dec_budget
    plan_budget = (enc_all_budget + dec_all_budget) * p.cct_bucket_p

    if p.cct_bucket_p > 0.0:
      loss += tf.abs(used_budget - plan_budget) / (plan_budget + 1e-8)
    else:
      loss += tf.abs(used_budget)
...
def train_fn(features, initializer=None):
  ...
  with tf.variable_scope(p.scope_name or "model",
    ...
    state = encoder(features['source'], p, gater)
    loss, logits, state, _ = decoder(features['target'], state, p, gater)
...
def score_fn(features, initializer=None):
  ...
  with tf.variable_scope(p.scope_name or "model",
    ...
    state = encoder(features['source'], p, gater)
    _, _, _, scores = decoder(features['target'], state, p, gater)
...
model.model_register("transformer_cct", train_fn, score_fn, infer_fn)
